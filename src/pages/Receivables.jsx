...
import { Plus, Trash2, ArrowDownCircle, Pencil, Check, X, Upload } from 'lucide-react'
import { getData, setData, generateId } from '../lib/storage'
import { formatCurrency } from '../lib/forecastEngine'
import CSVImport from '../components/CSVImport'

export default function Receivables() {
  const [items, setItems] = useState(() => getData('receivables', []))
  const [showForm, setShowForm] = useState(false)
  const [showImport, setShowImport] = useState(false)
  const [editId, setEditId] = useState(null)
  const [form, setForm] = useState({
    name: '', invoiceNumber: '', invoiceDate: '', amount: '', promiseDate: '', status: 'open', notes: ''
  })

  useEffect(() => { setData('receivables', items) }, [items])

  const resetForm = () => {
    setForm({ name: '', invoiceNumber: '', invoiceDate: '', amount: '', promiseDate: '', status: 'open', notes: '' })
    setShowForm(false)
    setEditId(null)
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (editId) {
      setItems(items.map(x => x.id === editId ? { ...entry, id: editId } : x))
    } else {
      setItems([...items, { ...entry, id: generateId() }])
    }
    resetForm()
  }
  ...
  return (
    <>
    {showImport && <CSVImport type="receivables" onClose={() => setShowImport(false)} onImport={(imported) => {
      const newItems = imported.map(item => ({ id: generateId(), customer: item.customer, description: item.description, amount: item.amount, promiseDate: item.promiseDate, status: 'open', notes: '' }));
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <ArrowDownCircle size={24} /> Accounts Receivable
          </h1>
          <p className="text-[#94a3b8] mt-1">Track incoming payments and expected collection dates</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="bg-[#1e293b] rounded-xl p-5 border border-[#334155]">
          <p className="text-sm text-[#94a3b8]">Open Invoices</p>
          <p className="text-2xl font-bold text-white mt-1">{openItems.length}</p>
        </div>
        <div className="bg-[#1e293b] rounded-xl p-5 border border-[#334155]">
          <p className="text-sm text-[#94a3b8]">Total Outstanding</p>
          <p className="text-2xl font-bold text-[#f59e0b] mt-1">{formatCurrency(totalOpen)}</p>
        </div>
        <div className="bg-[#1e293b] rounded-xl p-5 border border-[#334155]">
          <p className="text-sm text-[#94a3b8]">Collected</p>
          <p className="text-2xl font-bold text-[#22c55e] mt-1">{formatCurrency(paidItems.reduce((s, i) => s + i.amount, 0))}</p>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#1e293b] rounded-xl p-6 border border-[#334155] mb-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-[#64748b] mb-1">Customer Name</label>
            ...
            <div>
              <label className="block text-xs text-[#64748b] mb-1">Expected Payment Date</label>
            ...
            <div>
              ...
              <input type="text" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                className="w-full bg-[#0f172a] border border-[#334155] rounded-lg px-3 py-2 text-white text-sm focus:border-[#3b82f6] focus:outline-none" placeholder="Optional" />
            </div>
          </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#334155]">
              <th className="text-left text-xs font-medium text-[#64748b] uppercase px-6 py-3">Customer</th>
              ...
              <th className="text-left text-xs font-medium text-[#64748b] uppercase px-6 py-3">Expected</th>
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-12 text-[#64748b]">No receivables yet.</td></tr>
            ) : (
              items.map(item => (
                <tr key={item.id} className={`border-b border-[#334155]/50 hover:bg-[#334155]/20 ${item.status === 'paid' ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-6 py-3 text-[#94a3b8] text-sm">{item.promiseDate || '—'}</td>
                  <td className="px-6 py-3 text-center">
                    <span className={`text-xs px-2 py-1 rounded ${item.status === 'paid' ? 'bg-[#22c55e]/20 text-[#22c55e]' : 'bg-[#f59e0b]/20 text-[#f59e0b]'}`}>
                      {item.status === 'paid' ? 'Paid' : 'Open'}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-right flex items-center justify-end gap-1">
                    {item.status === 'open' && (
                      <button onClick={() => markPaid(item.id)} className="text-[#22c55e] hover:text-green-300 mr-1" title="Mark Paid">
                        <Check size={14} />
                      </button>
                    )}
